...
def make_qrc(data, caption, boxsize, color_text='black'):
    ...
    draw.text(((width - w) / 2, (height + ((height / 15) - h + 5) / 2)), caption, font=Imfont, fill=color_text)
    # split the caption into 2 lines and draw center
    if caption2 is not None:
        draw.text(((width - w2) / 2, (height + ((height / 15) + h2 + 5) / 2)), caption2, font=Imfont, fill=color_text)
